defmodule TradeMachine.Mailer.TradeSubmitEmail do
  ...
  @spec send(String.t(), String.t(), String.t(), String.t(), Ecto.Repo.t()) ::
          {:ok, any()} | {:error, any()}
  def send(trade_id, recipient_user_id, submit_url, frontend_environment, repo) do
    with hydrated_trade when not is_nil(hydrated_trade) <-
      ...
      nil ->
        Logger.error("Trade or user not found for trade submit email",
          trade_id: trade_id,
    ...
    e ->
      Logger.error("Database error sending trade submit email",
        trade_id: trade_id,
  ...
  @spec generate_email(HydratedTrade.t(), User.t(), String.t(), String.t()) ::
          Swoosh.Email.t()
  def generate_email(hydrated_trade, user, submit_url, frontend_environment) do
    to_email =
    ...
    recipient_count = length(hydrated_trade.recipients)

    title_text =
      if recipient_count == 1 do
        "#{hd(hydrated_trade.recipients)} accepted your trade proposal"
      else
