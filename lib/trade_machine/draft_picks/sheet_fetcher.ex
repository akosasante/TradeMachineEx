defmodule TradeMachine.DraftPicks.SheetFetcher do
  @moduledoc """
  Fetches draft picks data from a public Google Sheet as CSV.
  ...
  @doc """
  Fetches the draft picks sheet as CSV and returns the auto-decoded rows.
  ...
  def fetch(sheet_id, gid) do
    ...
    Logger.info("Fetching draft picks sheet", sheet_id: sheet_id, gid: gid)

    req =
      Req.new(url: url, redirect_log_level: false)
      |> Req.merge(
        Application.get_env(:trade_machine, :draft_picks_sheet_fetcher_req_options, [])
      )

    case Req.get(req) do
      {:ok, %{status: 200, body: rows}} when is_list(rows) ->
        Logger.info("Fetched draft picks sheet", row_count: length(rows))
        {:ok, rows}
      ...
      {:ok, %{status: status, body: body}} ->
        Logger.error("Failed to fetch draft picks sheet",
          status: status,
      ...
      {:error, reason} ->
        Logger.error("HTTP request failed for draft picks sheet", error: inspect(reason))
        {:error, reason}
    end
  end
  ...
  def fetch_from_config do
    sheet_id = Application.fetch_env!(:trade_machine, :draft_picks_sheet_id)
