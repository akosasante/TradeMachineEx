defmodule TradeMachine.MinorLeagues.SheetFetcherTest do
  ...
  describe "fetch/2" do
    test "returns {:ok, rows} when response body is a list (auto-decoded CSV)" do
      rows = [["Name", "Team", "Pos"], ["Joe Smith", "ATL", "OF"]]
    ...
    test "returns :unexpected_binary_body when body is a binary string" do
      # Use text/plain so Req does not attempt CSV or JSON decoding — body stays binary
      Req.Test.stub(SheetFetcher, fn conn ->
        conn
        |> Plug.Conn.put_resp_content_type("text/plain")
        |> Plug.Conn.resp(200, "Name,Team\nJoe Smith,ATL\n")
      end)
  ...
  describe "fetch_from_config/0" do
    test "delegates to fetch/2 using application config values" do
      Application.put_env(:trade_machine, :minor_league_sheet_id, "config_sheet_id")
      Application.put_env(:trade_machine, :minor_league_sheet_gid, "config_gid")

      rows = [["Player", "Org"], ["Test Player", "NYY"]]
    ...
    after
      Application.delete_env(:trade_machine, :minor_league_sheet_id)
      Application.delete_env(:trade_machine, :minor_league_sheet_gid)
    end
  end
end
